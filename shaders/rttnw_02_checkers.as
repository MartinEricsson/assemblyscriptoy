// ============================================================
//  Ray Tracing: The Next Week — Checkered Spheres
// ============================================================
//  Two large spheres with a spatial checker texture against a
//  solid sky. One path per frame accumulates in persistent memory.
// ============================================================
...
function checker(px: f32, py: f32, pz: f32): bool {
  const s: f32 = 1.0 / 0.32;
  const ix: i32 = <i32>Mathf.floor(s * px);
  const iy: i32 = <i32>Mathf.floor(s * py);
  const iz: i32 = <i32>Mathf.floor(s * pz);
  return ((ix + iy + iz) & 1) == 0;
}

function sphereCY(id: i32): f32 {
  return id == 0 ? 10.0 : -10.0;
}
  ...
  for (let i: i32 = 0; i < WIDTH * HEIGHT; i++) {
    ...
    for (let depth: i32 = 0; depth < MAX_DEPTH; depth++) {
      ...
      if (hitId < 0) {
        ...
        const outwardX: f32 = hx / 10.0;
        const outwardY: f32 = (hy - cy) / 10.0;
        const outwardZ: f32 = hz / 10.0;
        ...
        const even: bool = checker(hx, hy, hz);
        const albedoR: f32 = even ? 0.2 : 0.9;
        const albedoG: f32 = even ? 0.3 : 0.9;
        const albedoB: f32 = even ? 0.1 : 0.9;
