// ============================================================
//  Ray Tracing: The Next Week — Cornell Smoke
// ============================================================
//  Cornell walls with a larger dimmer light and two constant-
//  density media in place of the solid boxes. One path per frame
//  accumulates in persistent memory.
// ============================================================
...
function aabbNearFar(
  ...
  const tNear: f32 = Mathf.max(t0x, Mathf.max(t0y, t0z));
  const tFar: f32 = Mathf.min(t1x, Mathf.min(t1y, t1z));
  if (tNear >= tFar) return -1.0e20;
  return wantFar ? tFar : tNear;
}
...
function volumeHitT(
  ...
  const rdy: f32 = dy;
  const rdz: f32 = sinT * dx + cosT * dz;
  let t1: f32 = aabbNearFar(rox, roy, roz, rdx, rdy, rdz, minX, minY, minZ, maxX, maxY, maxZ, false);
  const t2: f32 = aabbNearFar(rox, roy, roz, rdx, rdy, rdz, minX, minY, minZ, maxX, maxY, maxZ, true);
  if (t1 < -1.0e19 || t2 < -1.0e19) return -1.0;
  if (t1 < tMin) t1 = tMin;
  let tExit: f32 = t2;
  if (tExit > tMax) tExit = tMax;
  if (t1 >= tExit) return -1.0;
  if (t1 < 0.0) t1 = 0.0;
  const dirLen: f32 = length3(dx, dy, dz);
  const distInside: f32 = (tExit - t1) * dirLen;
  let u: f32 = rand;
  if (u < 0.000001) u = 0.000001;
  const hitDist: f32 = NEG_INV * logF(u);
