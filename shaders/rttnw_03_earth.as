// ============================================================
//  Ray Tracing: The Next Week — Earth
// ============================================================
//  A lambertian globe sampled from a host-packed equirectangular
//  RGB8 texture. One path per frame accumulates in persistent memory.
// ============================================================
  ...
  for (let i: i32 = 0; i < WIDTH * HEIGHT; i++) {
    ...
    for (let depth: i32 = 0; depth < MAX_DEPTH; depth++) {
      ...
      if (t <= 0.0) {
        ...
        const hx: f32 = ox + dx * t;
        const hy: f32 = oy + dy * t;
        const hz: f32 = oz + dz * t;
        const outwardX: f32 = hx / 2.0;
        const outwardY: f32 = hy / 2.0;
        const outwardZ: f32 = hz / 2.0;
        ...
        const albedoR: f32 = <f32>(packed & 255) / 255.0;
        const albedoG: f32 = <f32>((packed >> 8) & 255) / 255.0;
        const albedoB: f32 = <f32>((packed >> 16) & 255) / 255.0;
