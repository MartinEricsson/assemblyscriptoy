// ============================================================
//  Ray Tracing in One Weekend — Dielectrics
// ============================================================
//  A hollow glass sphere (air bubble inside dielectric) sits
//  beside lambertian and fuzzy-metal balls. Refraction uses
//  Schlick reflectance; paths accumulate across frames.
// ============================================================
...
function sphereRadius(id: i32): f32 {
  if (id == 1) return 100.0;
  if (id == 3) return 0.4;
  return 0.5;
}

function sphereMat(id: i32): i32 {
  if (id == 2 || id == 3) return MAT_GLASS;
  if (id == 4) return MAT_METAL;
  return MAT_LAMBERT;
}
...
function sphereIor(id: i32): f32 {
  if (id == 3) return 1.0 / 1.5;
  return 1.5;
}
  ...
  for (let i: i32 = 0; i < WIDTH * HEIGHT; i++) {
    ...
    for (let depth: i32 = 0; depth < MAX_DEPTH; depth++) {
      ...
      if (mat == MAT_LAMBERT) {
        ...
        const uy: f32 = dy * inv;
        const uz: f32 = dz * inv;
        const eta: f32 = sphereIor(hitId);
        const ri: f32 = frontFace ? 1.0 / eta : eta;
